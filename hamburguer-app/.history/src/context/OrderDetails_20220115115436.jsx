...
const OrderDetailsProvider = (props) => {
    ...
    const setter = (id, newCount) => {
        const newProductsCount = updateProductCount(id, newCount, productCount);
        setProductsCount(newProductsCount);
    }
    const getterById = () => {
        return copyProductCount.filter((product)=>{return product.id === id})
    }
    const value = { productsCount, setter };
